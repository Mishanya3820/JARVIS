...
class JarvisApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.sidebar = ctk.CTkFrame(self, width=220, corner_radius=0, fg_color=PANEL, border_width=1, border_color=BORDER)
        self.sidebar.grid(row=0, column=0, sticky="nsew")
        self.sidebar.grid_propagate(False)

        brand = ctk.CTkFrame(self.sidebar, fg_color="transparent")
        brand.pack(fill="x", padx=22, pady=(28, 30))
        ctk.CTkLabel(brand, text="JARVIS", text_color=TEXT, font=ctk.CTkFont(size=28, weight="bold")).pack(anchor="w")
        ctk.CTkLabel(brand, text="LOCAL AI SYSTEM", text_color=ACCENT, font=ctk.CTkFont(size=10, weight="bold")).pack(anchor="w", pady=(2, 0))
        ...
        ctk.CTkFrame(self.sidebar, height=1, fg_color=BORDER).pack(fill="x", padx=18, pady=20)
        ...
        ctk.CTkLabel(bottom, text="Offline-first • Windows", text_color=MUTED, font=ctk.CTkFont(size=10)).pack(anchor="w")
    ...
    def _nav_button(self, text, command):
        return ctk.CTkButton(self.sidebar, text=text, command=command, anchor="w", height=42, corner_radius=9, fg_color="transparent", hover_color=PANEL_2, text_color=TEXT, font=ctk.CTkFont(size=13, weight="bold"))
    ...
    def _build_main_page(self):
        ...
        ctk.CTkLabel(title_box, text="Система JARVIS", text_color=TEXT, font=ctk.CTkFont(size=25, weight="bold")).pack(anchor="w")
        self.network_label = ctk.CTkLabel(title_box, text="Проверка сети...", text_color=MUTED, font=ctk.CTkFont(size=11))
        self.network_label.pack(anchor="w", pady=(2, 0))
        self.status_pill = ctk.CTkLabel(header, text="  ЗАГРУЗКА  ", text_color=MUTED, fg_color=PANEL, corner_radius=16, font=ctk.CTkFont(size=11, weight="bold"))
        self.status_pill.grid(row=0, column=1, sticky="e")
        hero = ctk.CTkFrame(page, fg_color=PANEL, corner_radius=18, border_width=1, border_color=BORDER)
        hero.grid(row=1, column=0, sticky="ew", pady=(0, 14))
        hero.grid_columnconfigure(1, weight=1)
        self.core_indicator = ctk.CTkLabel(hero, text="J", width=100, height=100, corner_radius=50, fg_color="#16283b", text_color=ACCENT, font=ctk.CTkFont(size=42, weight="bold"))
        self.core_indicator.grid(row=0, column=0, rowspan=2, padx=28, pady=24)
        ctk.CTkLabel(hero, text="CENTRAL INTELLIGENCE", text_color=MUTED, font=ctk.CTkFont(size=10, weight="bold")).grid(row=0, column=1, sticky="sw", pady=(22, 0))
        self.status_label = ctk.CTkLabel(hero, text="Загрузка компонентов...", text_color=TEXT, font=ctk.CTkFont(size=20, weight="bold"))
        self.status_label.grid(row=1, column=1, sticky="nw", pady=(3, 24))
        self.wake_status_label = ctk.CTkLabel(hero, text="Wake word: инициализация...", text_color=MUTED, font=ctk.CTkFont(size=11))
        self.wake_status_label.grid(row=0, column=2, padx=28, pady=(22, 0), sticky="e")
        self.log_box = ctk.CTkTextbox(page, fg_color=PANEL, border_width=1, border_color=BORDER, corner_radius=14, wrap="word", font=ctk.CTkFont(size=13))
        ...
        self.text_entry = ctk.CTkEntry(input_frame, height=46, corner_radius=12, fg_color=PANEL, border_color=BORDER, placeholder_text="Введите команду или вопрос...")
        self.text_entry.grid(row=0, column=0, sticky="ew", padx=(0, 8))
        self.text_entry.bind("<Return>", lambda event: self.on_send_text())
        self.send_button = ctk.CTkButton(input_frame, text="➤", width=54, height=46, corner_radius=12, command=self.on_send_text)
        self.send_button.grid(row=0, column=1, padx=(0, 8))
        self.mic_button = ctk.CTkButton(input_frame, text="🎙", width=54, height=46, corner_radius=12, command=self.on_mic_button, state="disabled")
        self.mic_button.grid(row=0, column=2)
        ctk.CTkLabel(page, text="Enter — отправить   •   🎙 — голосовой ввод   •   Wake word — «Джарвис»", text_color=MUTED, font=ctk.CTkFont(size=10)).grid(row=4, column=0, sticky="w", pady=(7, 0))
    ...
    def _build_settings_page(self):
        ...
        header = ctk.CTkFrame(page, fg_color="transparent")
        header.grid(row=0, column=0, sticky="ew", pady=(0, 14))
        ctk.CTkLabel(header, text="Настройки", text_color=TEXT, font=ctk.CTkFont(size=25, weight="bold")).pack(anchor="w")
        ctk.CTkLabel(header, text="Конфигурация голосовой системы и онлайн-ответов", text_color=MUTED).pack(anchor="w", pady=(2, 0))
        scroll = ctk.CTkScrollableFrame(page, fg_color=PANEL, corner_radius=16, border_width=1, border_color=BORDER)
        ...
        self.groq_key_entry = ctk.CTkEntry(key_frame, show="•", placeholder_text="Введите API-ключ")
        ...
        ctk.CTkButton(key_frame, text="Удалить", width=90, command=self._clear_groq_key).pack(side="right")
        ...
        ctk.CTkLabel(device_row, text="Устройство XTTS", text_color=MUTED).grid(row=0, column=0, sticky="w", padx=(0, 8))
        ctk.CTkLabel(device_row, text="Язык", text_color=MUTED).grid(row=0, column=1, sticky="w", padx=(8, 0))
        self.device_var = tk.StringVar(value=self.settings.get("xtts_device", "cpu"))
        self.device_menu = ctk.CTkOptionMenu(device_row, variable=self.device_var, values=["cpu", "cuda"])
        ...
        ctk.CTkLabel(scroll, text="Поля XTTS используются при следующем синтезе.", text_color=MUTED).pack(anchor="w", padx=4, pady=(0, 12))
        ...
        ctk.CTkLabel(scroll, text="Порог и устройство Rustpotter сейчас задаются в settings.json.", text_color=MUTED).pack(anchor="w", padx=4, pady=(0, 18))
        self.settings_status_label = ctk.CTkLabel(scroll, text="", text_color=GOOD)
        self.settings_status_label.pack(pady=(0, 16))

    def _section(self, parent, text):
        ctk.CTkLabel(parent, text=text, text_color=ACCENT, font=ctk.CTkFont(size=11, weight="bold")).pack(anchor="w", padx=4, pady=(8, 10))
    ...
    def _show_first_run_groq_setup(self):
        ...
        dialog.grab_set()
        ctk.CTkLabel(dialog, text="Добро пожаловать в JARVIS", font=ctk.CTkFont(size=22, weight="bold")).pack(pady=(24, 8))
        ctk.CTkLabel(dialog, text="Для онлайн-ответов нужен API-ключ Groq.\nЕго можно добавить сейчас или позже в настройках.", justify="center").pack(pady=(0, 18))
        ...
        ctk.CTkButton(buttons, text="Пропустить", command=dialog.destroy, fg_color="#343b45", hover_color="#424b57").pack(side="right", expand=True, padx=5)
    ...
    def _start_wake_word(self):
        try:
            ...
            self.wake_detector = RustpotterWakeWordDetector(on_wake=self._on_wake_word_detected, cli_path=cli_path, model_path=model_path, threshold=float(self.settings.get("wake_word_threshold", 0.5)), device_index=int(self.settings.get("rustpotter_device_index", 0)))
            self.wake_detector.start()
            self._set_component("wake", True)
            self.after(0, lambda: self.wake_status_label.configure(text='Wake word активен • скажите «Джарвис»', text_color=GOOD))
        except Exception as e:
            self._set_component("wake", False)
            print(f"[WakeWord] Не удалось запустить: {e}")
            self.after(0, lambda err=str(e): self.wake_status_label.configure(text=f"Wake word недоступен: {err}", text_color=BAD))
    ...
    def _stop_wake_word(self):
        ...
        if hasattr(self, "wake_status_label"):
            self.after(0, lambda: self.wake_status_label.configure(text="Wake word выключен", text_color=MUTED))
    ...
    def _process_and_reply(self, text: str, grammar_text: str | None = None):
        try:
            ...
            reply = jarvis_core.process_message(text, grammar_text=grammar_text, on_speak_ready=_on_speak_ready)
            if reply["type"] == "sound":
