        ...
        with _tts_lock:
            if _tts is None:
                try:
                    ...
                    _IMPORT_ERROR = exc
                    raise RuntimeError(
                        "Библиотека coqui-tts не установлена (пакет называется "
                        "именно 'coqui-tts', НЕ 'TTS' — старая 'TTS' заброшена). "
                        "Установи: pip install coqui-tts"
                    ) from exc
 
                device = _get_device()
                print(f"[XTTS] Каталог моделей: {TTS_MODELS_DIR}")
                print(f"[XTTS] Загружаю модель {_MODEL_NAME} на {device}...")
                # Примечание: специального обхода torch.load(weights_only=...)
                # здесь больше не нужно — coqui-tts сам регистрирует все
                # нужные классы (XttsConfig, XttsAudioConfig, BaseDatasetConfig,
                # XttsArgs) через torch.serialization.add_safe_globals(...)
                # прямо при "import TTS" (см. TTS/__init__.py в пакете
                # coqui-tts), если torch >= 2.4. Раньше это приходилось
                # патчить вручную — форк уже решил это за нас.
