...
def _ensure_gigaam_installed() -> None:
    ...
    install_gigaam(
        lambda name, current, total: print(
            f"[GigaAM] {name}: {current / total * 100:.0f}%" if total else f"[GigaAM] {name}"
        )
    )
...
def play_sound(path: str) -> None:
    ...
    try:
        ...
        padding = np.zeros((padding_frames, channels), dtype=np.float32) if channels > 1 else np.zeros(padding_frames, dtype=np.float32)
